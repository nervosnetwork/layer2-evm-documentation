...
mkdir -p ~/projects
cd ~/projects
curl -LO https://github.com/nervosnetwork/ckb-cli/releases/download/v0.101.0/ckb-cli_v0.101.0_x86_64-unknown-linux-gnu.tar.gz
tar xvzf ckb-cli_v0.101.0_x86_64-unknown-linux-gnu.tar.gz
mv ckb-cli_v0.101.0_x86_64-unknown-linux-gnu ckb-cli-0.101.0
```
...
mkdir -p ~/projects
cd ~/projects
curl -LO https://github.com/nervosnetwork/ckb-cli/releases/download/v0.101.0/ckb-cli_v0.101.0_x86_64-apple-darwin.zip
unzip ckb-cli_v0.101.0_x86_64-apple-darwin.zip
mv ckb-cli_v0.101.0_x86_64-apple-darwin ckb-cli-0.101.0
```
...
cd ~/projects
Invoke-WebRequest https://github.com/nervosnetwork/ckb-cli/releases/download/v0.101.0/ckb-cli_v0.101.0_x86_64-pc-windows-msvc.zip -OutFile ckb-cli_v0.101.0_x86_64-pc-windows-msvc.zip
Expand-Archive .\ckb-cli_v0.101.0_x86_64-pc-windows-msvc.zip
New-Item -ItemType Directory -Force -Path .\ckb-cli-0.101.0
Get-ChildItem -Path .\ckb-cli_v0.101.0_x86_64-pc-windows-msvc\ckb-cli_v0.101.0_x86_64-pc-windows-msvc\ -Recurse -File | Move-Item -Destination ckb-cli-0.101.0
Get-ChildItem -Path .\ckb-cli_v0.101.0_x86_64-pc-windows-msvc\ -Recurse -Directory | Remove-Item
.\ckb-cli-0.101.0\ckb-cli.exe
```

The `ckb-cli` application should now exist in the `~/projects/ckb-cli-0.101.0` directory. You can execute it by running `~/projects/ckb-cli-0.101.0/ckb-cli` from any folder, or by running `./ckb-cli` if you are already in the `~/projects/ckb-cli-0.101.0` directory.
...
```
~/ckb-cli-0.101.0$ ./ckb-cli --url http://3.235.223.161:18114
...
[  ckb-cli version ]: 0.101.0 (1b66835 2021-10-20)
[              url ]: http://3.235.223.161:18114 (network: Testnet)
[              pwd ]: /home/username/projects/ckb-cli-0.101.0
[            color ]: true
[            debug ]: false
[          no-sync ]: false
[    output format ]: yaml
