...
Before you begin on this task you must complete the [first](../tasks/1.create.godwoken.account.md) and [second](../tasks/2.deploy.eth.contract.md) tasks. If you have not completed them, please do so now.
...
This task requires the Layer 2 EVM Documentation repo ([layer2-evm-documentation](git@github.com:nervosnetwork/layer2-evm-documentation.git)) which was setup in [task 2](../tasks/2.deploy.eth.contract.md#2-clone-and-setup-the-gitcoin-task-instruction-examples). If you do not have this repo available for any reason, please set it up now.
...
In the [previous task](../tasks/2.deploy.eth.contract.md), you compiled and deployed an Ethereum smart contract. You may be able to reuse that smart contract for this task. If it is no longer available, please revisit the [previous task](../tasks/2.deploy.eth.contract.md) and complete it again.
...
Next we will use the example code to make a function call in your smart contract. Open the file `layer2-evm-documentation/code-examples/3-call-contract/index.js` in an editor of your choosing, and find the `readCall()` and `writeCall()` functions.
...
```sh
cd ~/projects/layer2-evm-documentation/src/examples/3-call-contract
node index.js
```
